...
void
setzero_pcalc (struct probcalc *pcalc)
{
  // can use memset because it is a simple array of zeros
  memset (pcalc->qintegrate, 0, numpopsizeparams * sizeof (int));
  if (nummigrateparams > 0)
  {
    // can use memset because it is a simple array of zeros
    memset (pcalc->mintegrate, 0, nummigrateparams * sizeof (double)); 
  }
  pcalc->sintegrate = 0;
  pcalc->pdg = 0;
  pcalc->plg = 0;
  ...
  if (modeloptions[NOMIGRATION] == 0)
  {
    for (k = 0; k < lastperiodnumber; k++)
    {
      for (i = 0; i < npops - k; i++)
      {
        ...
        for (j = 0; j < npops - k; j++)
          assert (dest->mc[k][i][j] == 0 || dest->fm[k][i][j] > 0);
      }
    }
  }
  return;
...
// compared memset with loops and memset did ok
void
clear_treeinfo (struct genealogy_weights *dest) // slightly slower than setzero_treeinfo
{
  int k, i;
  for (i = 0; i < numsplittimes + 1; i++)
  {
    memset (dest->cc, 0, npops - i * sizeof (int));
    memset (dest->hcc, 0, npops - i * sizeof (double));
    memset (dest->fc, 0, npops - i * sizeof (double));
  }
  if (modeloptions[NOMIGRATION] == 0)
  {
    for (k = 0; k < lastperiodnumber; k++)
    {
      for (i = 0; i < npops - k; i++)
      {
        memset (dest->mc[k][i], 0, (npops - k) * sizeof (int));
        memset (dest->fm[k][i], 0, (npops - k) * sizeof (double));
      }
    }
  }
  return;
...
copy_probcalc (struct probcalc *dest, struct probcalc *srce)
{
  ...
  /* Note that neither pdg nor plg is copied!!! */
                       ...
                       struct genealogy_weights *addtominus)
{
  ...
  for (i = 0; i < numsplittimes + 1; i++)
  {
    for (j = 0; j < npops - i; j++)
    {
      ...
      //      if (addup->cc[i][j]==0 && addup->fc[i][j] < subminval)
      //      addup->fc[i][j] = 0;       // necessary ?
    }
  }
    ...
    for (k = 0; k < lastperiodnumber; k++)
    {
      for (i = 0; i < npops - k; i++)
      {
        for (j = 0; j < npops - k; j++)
        {
          ...
          //   if (addup->mc[k][i][j] ==0 && addup->fm[k][i][j] < subminval)
          //   addup->fm[k][i][j] = 0;     // necessary ?
        }
      }
    }
  }
...
calc_gsampinf_length (void)
{
  ...
  i += 3 + modeloptions[SPLITTINGRATEPARAMETER];        // sintegrate pdg, plg, and probg 
  return i;
}

void
savegsampinf (float *g)
{

  int i, j, c;
  float f, hc;
  // positions where these types (mc,fc,mintegrate, qintegrate, sintegrate) begin in array g
  struct genealogy_weights *gweight = &C[0]->allgweight;
  for (i = 0; i < numpopsizeparams; i++)
  {
    c = 0;
    f = hc = 0.0;
  ...
  for (i = 0; i < numpopsizeparams; i++)
  {
    g[gsamp_qip + i] = (float) C[0]->allpcalc.qintegrate[i];
  }
  for (i = 0; i < nummigrateparams; i++)
  {
    g[gsamp_mip + i] = (float) C[0]->allpcalc.mintegrate[i];
  }
  if (modeloptions[SPLITTINGRATEPARAMETER])
    g[gsamp_sip] = (float) C[0]->allpcalc.sintegrate;
  g[gsamp_pdgp] = (float) C[0]->allpcalc.pdg;
  g[gsamp_plgp] = (float) C[0]->allpcalc.plg;
  g[gsamp_probgp] = (float) C[0]->allpcalc.probg;
  for (i = 0; i < numsplittimes; i++)
    g[gsamp_tp + i] = (float) C[0]->tvals[i];
}                               /* savesampinf */
