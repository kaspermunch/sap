...
aread (FILE * mcffile, const char *name, int atype, int iucheck, void *a)
{
  ...
  fscanf (mcffile, "%s %d %d ", namecheck, &typecheck, &iu);
...
writemcf (char mcffilename[])
{
  ...
  for (ci = 0; ci < numchains; ci++)
  {
    ...
    for (li = 0; li < nloci; li++)
    {
      ...
      /* should not be necessary as there get initialized anyway
...
- if cc > cf,  then the first cf chains in the mcffile are loaded into the first cf positions in C[]
	then the mcffile is closed and reopnened and the chains are reloaded in to the next available positions
    ...
    if ((checkeofc = (char) fgetc (mcffile)) == EOF)
    {
      if (ci == lastci)
        IM_err (IMERR_MCFSPLITTIMEPROB,
                " can't load trees, probably because of splittime time conflict with t prior");
      if (ci < numchains - 1)
      {
        f_close (mcffile);
        if ((mcffile = fopen (mcffilename, "r")) == NULL)
