         ...
         int nomigration)
{
  ...
  int i, j;
  //int periodi = 0; not used once
  //int done = 0; not used once
  int p[2];
  double nt[2];
  double start_coalesce_rate;

  // set small value for coalescent rate relative to splitting times when building initial tree
  ...
  {
    start_coalesce_rate = curgenes / tprior;
    for (i = 0; i < 2; i++)
    {
      j = gtree[c[i]].pop;
...
makeHKY (int ci, int li)
{
  ...
  for (i = L[li].numgenes; i < 2 * L[li].numgenes - 1; i++)
  {
    ...
    gtree[i].hkyi.frac = malloc (L[li].numsites * sizeof (double *));
    gtree[i].hkyi.newfrac = malloc (L[li].numsites * sizeof (double *));
    for (j = 0; j < L[li].numsites; j++)
    {
      gtree[i].hkyi.frac[j] = malloc (4 * sizeof (double));
      gtree[i].hkyi.newfrac[j] = malloc (4 * sizeof (double));
    }
  }
  C[ci]->G[li].mignum = 0;
  ptime = 0.0;
  curgenes = L[li].numgenes;
  distmat = alloc2Ddouble (L[li].numgenes, L[li].numgenes);
  ...
  do
  {
    ...
    c[0] = ids[ai];
    c[1] = ids[aj];
    addedge (ci, li, newedge, c, curgenes, &ptime,
             modeloptions[NOMIGBETWEENNONSISTERS]
             || modeloptions[NOMIGRATION]
  ...
  XFREE (edgevec);
  free2D ((void **) distmat, L[li].numgenes);
  XFREE (ids);
  C[ci]->G[li].hilike = -1e20;
}                               /* makeHKY */

void
makeIS (int ci, int li)
{
  ...
  gtree[C[ci]->G[li].root].mig[0].mt = -1;
  gtree[C[ci]->G[li].root].time = TIMEMAX;
  free2D ((void **) tempseq, L[li].numgenes);
  free2D ((void **) distmat, L[li].numgenes);
  XFREE (curid);
  XFREE (singletons);
  C[ci]->G[li].hilike = -1e20;
...
makeJOINT_IS_SW (int ci, int li)
{
  ...
  int sumd = 0, nd0, nd1;
  distmat = alloc2Dint (L[li].numgenes, L[li].numgenes);
  distmatA = alloc2Ddouble (L[li].numgenes, L[li].numgenes);
  ...
  free2D ((void **) tempseq, L[li].numgenes);
  free2D ((void **) distmat, L[li].numgenes);
  free2D ((void **) distmatA, L[li].numgenes);
  XFREE (curid);
  XFREE (singletons);
  C[ci]->G[li].hilike = -1e20;
}                               /* makeJOINT_IS_SW */
...
/*Not really sure how best to do the SW trees.  here is the rough procedure 
