...
static void integrate_autoc (FILE * outto, struct autoc ta[], double ac[],
                             int pstep);
void printautoctable (FILE * outto, int numautoc, struct autoc **ac,
                      char **ac_str, const char *printacstring, int pstep);
void iautoc (struct autoc *a);  // set autoc values to zero
void set_autoc_vals (double *pautoc_vals);
...
void
integrate_autoc (FILE * outto, struct autoc ta[], double ac[], int pstep)
{
...
otherwise we use calculated autocorrelation measured for a lag of 1 */
  ...
  if (stopsum)
    fprintf (outto, "\t%.0f", pstep / (1 + 2 * ess));
  else
    fprintf (outto, "\t< %.0f", pstep / (1 + 2 * ess));
}                               //integrate_autoc 
...
void
set_autoc_vals (double *pautoc_vals)
{
  int j, li, i = 0;
  if (lpgpd_v->do_autoc == 1)
  {
    pautoc_vals[i] = C[0]->allpcalc.pdg + C[0]->allpcalc.plg + C[0]->allpcalc.probg;
    i++;
  }
  for (j = 0; j < numsplittimes; j++)
    if (T[j].v->do_autoc == 1)
    {
      pautoc_vals[i] = C[0]->tvals[j];
      i++;
    }
  for (li = 0; li < nloci; li++)
    if (L[li].g_rec->v->do_autoc == 1)
    {
      pautoc_vals[i] = C[0]->G[li].roottime;
      i++;
    }
}                               //set_autoc_vals
...
set_autoc_a_vals (double *pautoc_vals)
{
  ...
  for (li = 0; li < nloci; li++)
  {
    ...
    {
      pautoc_vals[i] = IMA_assignment2value (0, li);
      i++;
    }
  }
}                               //set_autoc_vals
...
*/
void
checkautoc (int start_autocorrelations, int burndone, int burnsteps)
{
  int i, j;
...
void
callprintautoctable (FILE * outto, int pstep)
{
  ...
  if (autoc_pointer[0][0].cov.n > (AUTOCCUTOFF / AUTOCSTEPSCALAR))      // check one of the autocorrelation accumulators to see if enough counts have been made
  {
        ...
        && assignmentoptions[POPULATIONASSIGNMENTASSIGNED] == 0)
    {
      printautoctable (outto, num_autoc_a, autoc_a_pointer,
                       autoc_a_str_pointer,
                       "Population Assignment Autocorrelations and Effective Sample Size Estimates",
                       pstep);
    }
  }
}                               // callprintautoctable 
